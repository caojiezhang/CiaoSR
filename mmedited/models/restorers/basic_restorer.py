...
class BasicRestorer(BaseModel):
        ...
        for metric in self.test_cfg.metrics:
            if 'convert_to' in self.test_cfg:
                eval_result[metric] = self.allowed_metrics[metric](output, gt,
                                    crop_border=crop_border, convert_to=self.test_cfg.convert_to)
            else:
    ...
    def forward_test(self,
        ...
        Returns:
            dict: Output results.
        """
        if self.test_cfg.get('window_size', None):
            output = self.clip_generator(lq)
        else:
            output = self.generator(lq)
    ...
    def clip_generator(self, img):

        window_size = self.test_cfg.get('window_size')
        scale = self.test_cfg.get('upscale')

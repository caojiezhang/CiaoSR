...
class SmallUNetDiscriminatorWithSpectralNorm(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels=64, skip_connection=True):
        ...
        self.conv_1 = spectral_norm(
            nn.Conv2d(mid_channels, mid_channels * 1, 4, 2, 1, bias=False))
        self.conv_2 = spectral_norm(
            nn.Conv2d(mid_channels * 1, mid_channels * 1, 4, 2, 1, bias=False))
        # self.conv_3 = spectral_norm(
        #     nn.Conv2d(mid_channels * 1, mid_channels * 1, 4, 2, 1, bias=False))

        # upsample
        # self.conv_4 = spectral_norm(
        #     nn.Conv2d(mid_channels * 1, mid_channels * 1, 3, 1, 1, bias=False))
        self.conv_5 = spectral_norm(
            nn.Conv2d(mid_channels * 1, mid_channels * 1, 3, 1, 1, bias=False))
        self.conv_6 = spectral_norm(
            nn.Conv2d(mid_channels * 1, mid_channels, 3, 1, 1, bias=False))
...
if __name__ == '__main__':
    ...
    model = SmallUNetDiscriminatorWithSpectralNorm(in_channels=3, mid_channels=64, skip_connection=True)
    print(model(x).shape)
